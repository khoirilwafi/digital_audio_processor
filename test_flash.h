...
const uint8_t   pid[10] = { 0x05, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18 };
const uint16_t name[10] = { 5, 10, 20, 40, 80, 16, 32, 64, 128, 156 };
...
int main(void)
{
  ...
  HAL_Delay(1000);
  	...
  	if(HAL_I2C_IsDeviceReady(&hi2c2, 0x27<<1, 10, 100) != HAL_OK)
	{
		Error_Handler();
	}
	...
	if(id == 0x00)
	{
		lcd_set_cursor(0,0); lcd_print("FLASH           ");
	...
	if((id>>8) != 239)
	{
		lcd_set_cursor(0,0); lcd_print("FLASH TERDETEKSI");
		lcd_set_cursor(0,1); lcd_print("BUKAN WINBOND   ");

		Error_Handler();
	}

	uint16_t chip_name = 0;
	...
	lcd_set_cursor(0,0); lcd_print("CHIP:W25Q");
	lcd_set_cursor(9,0); lcd_int(chip_name);

	lcd_set_cursor(0,1); lcd_print("MID:");
	lcd_set_cursor(4,1); lcd_int(id>>8);

	lcd_set_cursor(8,1); lcd_print("PID:");
	lcd_set_cursor(12,1); lcd_int((uint8_t)id);
